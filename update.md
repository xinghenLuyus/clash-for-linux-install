...
- Go TUI 子页面一致性修复：
  - 统一右侧焦点态的 `↑↓` 行为：进入页面后方向键只移动右侧当前页面内容，不再混入左侧导航逻辑。
  - 总览、日志、内核、Web 面板在焦点态显示统一的动作列表与选中态，避免按键变化不可见。
  - 订阅空状态也会显示执行结果，新增订阅取消、失败或成功后不会像“无响应”。
  - 新增订阅继续复用 `clashsub add`；可选择添加后立即使用，等价于 `clashsub add --use <url>`，避免 TUI 与 CLI 订阅逻辑脱轨。
  - 刷新 Go TUI 本地二进制与 Linux `amd64` / `386` 预编译产物。
- Go TUI 订阅体验修复：
  - 订阅列表读取改为类型安全的 yq 表达式，并增加 Go 侧 `resources/profiles.yaml` 兜底解析，避免 `clashctl sub list` 有数据但 TUI 显示空列表。
  - 订阅列表读取失败时在页面内显示明确异常，不再误报为“暂无订阅”。
  - 新增、切换、更新、删除订阅等耗时动作执行前显示居中“处理中”模态，提示正在下载/转换/合并/重启，命令完成后自动回到当前页面。
  - 修正 shell TUI 预览中的订阅 id/use 类型比较，保持当前订阅标记一致。
  - 增加 profiles.yaml 兜底解析单测，覆盖数字/字符串 id 与当前订阅标记。
- 订阅下载代理环境修复：
